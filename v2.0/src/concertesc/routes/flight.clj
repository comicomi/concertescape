...
(defn send-flight-request [origin_code destination_code departure_date arrival_date]
  (let [body (create-request-body origin_code destination_code departure_date arrival_date)]
...
(defn request-flight [origin_code destination_code departure_date arrival_date]
  (parse-response (send-flight-request origin_code destination_code departure_date arrival_date)))

(defn process-connection [connection]
...
(defn process-response [body]
  (let [trip-option  (-> body :trips :tripOption first)
    ...
    (swap! result conj {:flight (map process-connection  (map :segment fares ))})))
